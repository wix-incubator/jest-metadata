...
export type MetadataCheckerConfig = {
...
export class MetadataChecker {
  constructor(private readonly config: MetadataCheckerConfig) {}
  ...
  private _assert<K extends keyof MetadataCheckerConfig>(
    ...
    if (!(metadata instanceof Klass)) {
      throw new TypeError(
        `Metadata (${metadata?.[symbols.id]}) is not an instance of ${klassName}`,
      );
    }
  }
}
